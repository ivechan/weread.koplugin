...
1. 前往 [GitHub Releases](https://github.com/ivechan/weread.koplugin/releases) 下载最新的 `weread.koplugin-vX.Y.Z.zip` 安装包。
...
KOReader 顶部菜单栏新增书本图标，点击直接打开微信读书书架；文件浏览器和阅读界面均可使用。
...
│   │   ├── 自动衔接下一章（默认开启，读完自动下载并打开下一章）
│   │   ├── 自动衔接上一章（默认开启，章节开头再往前翻会打开上一章末尾）
│   │   └── 章节预下载
│   │       ├── 自动预下载下一章（默认关闭，后台下载）
