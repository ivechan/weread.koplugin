# Changelog

在发布新版本前，将面向用户的中文更新说明添加到对应版本号下。各项更新直接并列展示，不再按“新功能”或“优化与修复”等小章节分类。发布工作流只会提取与 `_meta.lua` 当前版本完全匹配的段落。

## [1.5.8]

- 新增 KOReader 顶部菜单栏书架入口，点击书本图标直接打开微信读书书架，文件浏览器与阅读界面均可使用。

## [1.5.7]

- 新增“设置 → 同步进度提醒”开关，默认开启；关闭后自动进度同步仍在后台执行，不再显示同步成功或失败提示，修改立即生效。

## [1.5.6]

- 调整智能进度同步：取消本地进度必须领先服务器的限制；两者进度差小于 3% 时，以 KOReader 当前阅读位置同步至微信读书。

## [1.5.5]

- 修复切换章节遇到未完成同步请求时，新的进度同步被跳过的问题。
- 修复关闭书籍、休眠后同步状态及定时任务未正确清理的问题，恢复阅读时重新检查进度。
- 修复缓存书籍缺少网页阅读会话时自动同步静默跳过的问题，改为后台获取会话后继续同步。

## [1.5.4]

- 智能进度同步完成后会提示同步成功或失败。

## [1.5.3]

- 新增智能进度同步：翻到下一章或阅读中每隔 5 分钟，自动拉取微信读书服务器进度并与本地比对；当两者相差小于 3% 且本地更靠前时，后台异步回传进度。全程非阻塞、不影响阅读，休眠或关闭书籍时自动停止。

## [1.5.2]

- 优化书架封面网格：圆角封面卡片、私密阅读角标，公众号文章封面改为完整显示。
- 优化划线章节对应选择界面，匹配状态（待获取/未对应/已占用/可选择）和操作提示更清晰。
- 修复部分书籍获取划线和想法时中途报错的问题。
- 修复获取中断后，已完成章节的划线未及时显示的问题。
- 修复关闭书架后封面资源未释放的问题。

感谢 @IswordSun、@qiuyukang 的贡献。

## [1.5.1]

- 新增章节对应管理，支持手动调整匹配、按章获取或重新获取划线和想法，并显示“已获取”状态。
- 优化获取想法时的暂停与断点续传，保留已完成进度。
- 开书不再自动匹配划线位置；仅在章节预下载和预下载划线想法同时开启时，才在后台获取数据。
- 新增独立的本地 Mock 测试模式，方便在 macOS 模拟器和 Kindle 上验证功能。

## [1.5.0]

- 重做书架导航，支持书籍分组筛选，优化搜索和刷新体验。
- 整本下载支持断点续传，降低内存占用，并改进进度反馈与取消操作。
- 修复本地书章节匹配与划线刷新问题，减少开书和翻页时的卡顿。
- 修复部分扫码登录失败的问题。
- 自动检查更新的间隔缩短为 1 小时。

感谢 @IswordSun、@vein-cyber 的贡献。

## [1.4.8]

- 新增「启动时打开书架」开关（默认关闭）：开启后 KOReader 启动时直接进入微信读书书架。
- 章节开头再往前翻（上一页）会打开上一章并停在末尾；新增「自动衔接上一章」开关（默认开启）。

## [1.4.7]

- 插件自更新改为从本仓库（ivechan/weread.koplugin）的 GitHub Release 检查与下载。

## [1.4.6]

- 预下载开始与完成不再弹出提示，仅在预下载失败时提示，并延长提示停留时间，减少对阅读的打扰。

## [1.4.5]

- 降低后台预下载对阅读的干扰：预下载启动不再在 UI 线程做完整垃圾回收，图片在 EPUB 中不再重复压缩；可用内存偏低时跳过后台预下载，改由读完自动衔接按需下载。

## [1.4.4]

- 读完一章后自动衔接下一章：未缓存时会在前台静默下载并自动打开，不再需要手动点“下载”。
- 下一章预下载的内存门槛按设备总内存自适应，修复 256M 设备上因可用内存不足而几乎总是跳过预下载的问题。
- 后台预下载不可用时，改为在读完章节时按需前台下载，保证阅读连续。

## [1.4.3]

- 章节目录支持上千章的长书：打开和滚动不再卡顿，内存占用保持稳定。
- 章节目录新增快速定位：可跳到当前阅读章节、按章节号跳转或搜索章节标题；打开目录时自动定位到当前阅读章节。
- 整本下载支持章节级断点恢复：网络错误、取消或设备重启后再次下载会跳过已完成章节；打包前会重新逐章校验，避免生成残缺 EPUB 或覆盖已有完整 EPUB。
- 整本下载改为从章节暂存文件流式打包 EPUB，降低长书下载期间的内存占用。
- 章节匹配改为树状目录、父子联动多选，并修复划线和想法叠加层未及时刷新的问题。
- 修复本地书与微信读书章节顺序不一致时的划线和想法匹配问题。
- 修复二维码登录时验证码为空导致登录失败的问题。

感谢 @IswordSun 和 @vein-cyber 的贡献。

## [1.4.2]

- 优化划线和想法的拉取速度与进度显示，减少网络请求。
- 提升显示划线和想法时的翻页流畅度。
- 修复清理想法不彻底，以及重新打开书籍后重复匹配的问题。

## [1.4.1]

- 修复划线和想法的显示状态及章节匹配问题，兼容本地书与微信读书章节编号不一致的情况。
- 降低后台预下载和匹配的内存要求，并修复清理书籍缓存后旧划线想法自动恢复的问题。

## [1.4.0]

- 统一本地书与微信下载书的划线和想法管理，支持按章匹配、中断续传和同书数据复用。
- 下一章正文及划线想法改为后台预下载，减少阅读时的等待和卡顿。
- 优化公众号图片下载，降低内存占用。
- 修复部分书籍正文不显示、脚注识别异常的问题。

感谢 @baily-zhang、@szey 和 @q107580018 的贡献。

## [1.3.1]

- 整本下载的 EPUB 新增书籍简介，可在 KOReader 的书籍信息中查看。
- 优化自适应封面书架布局，修复部分屏幕上的内容溢出和滚动条问题，并用封面折角标记已下载书籍。

## [1.3.0]

- 新增自适应封面书架（设置-书架视图-封面模式），并默认启用分页，提升大书架的加载速度。
- 重做想法弹窗，支持 Emoji、长内容滚动、物理按键操作和显示样式调整，支持长按复制想法。
- 下载设置新增脚注弹窗选项，可隐藏正文中的脚注文本。
- 移除划线末尾的星号，避免影响正文排版。
- 修复阅读进度自动同步偶尔失败、封面书架显示不稳定等问题。

感谢 @lostanother、@jqs7 和 @baily-zhang 的贡献。

## [1.2.0]

### 新功能

- 本地书支持同步微信读书的划线与想法，无需重新生成 EPUB。
- 同步划线想法支持取消和断点续传，并可一键显示或隐藏划线。

### 优化与修复

- 整本下载失败时自动重试，不再保存残缺 EPUB。
- weread 主菜单简化和重构。

感谢 @lostanother、@Mr54233 的贡献。

本地书划线与想法功能参考了 https://github.com/Mr54233/pickthought.koplugin 的实现。

## [1.1.0]

### 新功能

- 支持使用方向键、翻页键和确认键操作书架、书籍详情、章节列表、书评与阅读统计，方便无触屏或主要依赖物理按键的设备使用。
- 新增可绑定到手势或按键的“微信读书·书架”“微信读书·本地书架”“微信读书·阅读统计”和“微信读书·搜索”动作；阅读界面继续提供快捷菜单。

### 优化与问题修复

- 优化大书和图片较多书籍的下载方式，显著降低内存占用，并在完成、失败或取消后自动清理临时文件。
- 脚注改为按 KOReader 默认方式显示在页面底部，解决脚注看不到或点击后跳错位置的问题。
- 改进划线与想法显示：长标题可以换行，想法标记不再被划线遮住。
- 修复空书架、空书评以及快速切换书籍时可能导致的闪退。
- 修复从微信读书书架打开图书后，书架残留并阻挡 KOReader 退出的问题。

> **升级提示：** 脚注显示方式会写入下载的书籍，已经下载的旧书需要重新下载才能使用新的脚注效果。

感谢 @SuzyZhang-Dev、@MsReverie、@jsfaint、@jqs7 和 @ViggoC 的贡献。

## [1.0.0]

### 新功能

- 全面重构书架、书籍详情和章节列表界面，优化书籍与公众号的浏览、缓存状态和操作入口。
- 支持 SimpleUI 和 ZenUI 快速启动，并增加专用图标与稳定的第三方调用入口。
- 支持将已下载书籍添加到 KOReader 本地书架，无网络时也可快速访问。
- 支持插件内手动或每日自动检查更新，查看更新日志、下载进度并在线安装。
- 更新下载默认优先使用 GitHub 代理，失败后自动回退直连。

### 优化与问题修复

- 增加章节目录的文件缓存与 SQLite 双重存储、自动回填和联网修复，解决目录缺失后阅读进度同步、阅读时长上报及章节识别失败的问题。
- 整理“关于”与更新选项的菜单层级，并优化菜单交互；关闭弹框或临时页面后保留原来的菜单位置。
- 恢复书架页的文字操作栏，改进小屏设备上的标题和操作区布局。
- 加强在线升级安全性：校验 SHA-256，限制更新包最大为 10 MiB，下载过程超限时立即终止，并在新版本成功启动后自动清理上一版备份。

感谢 @MsReverie 的贡献。

## [0.6.0]

- 支持自动预下载下一章。
- 支持批量勾选多个章节下载。
- 支持显示划线和想法。
- 支持通过手势打开微信读书快捷菜单。

感谢 @jqs7 的贡献。

## [0.5.1]

- 重构和拆分主模块，统一日志模块并增加 HTTP 错误输出。
- 增加 CI 流程卡点和自动化发布。
- 修复想法弹窗在旧版 KOReader 中单页只显示一条内容的问题。

感谢 @shichen35 的贡献。

## [0.5.0]

- 优化划线和想法：
  - 改用 KOReader 原生 UI 显示。
  - 大幅提升首次加载和显示速度，降低内存占用。
  - 增加划线边缘防误触。
- 支持阅读进度同步。
- 优化阅读时间上报逻辑，避免阻塞 UI。
- 使用 SQLite 存储想法，并保留 JSON 回退机制。

> **升级注意：** 本地划线和想法机制已经升级，必须重新下载划线和想法。想法显示界面改为 KOReader 原生 UI 后不再支持 Emoji 显示。

感谢 @shichen35、@MsReverie 和 @jqs7 的贡献。

## [0.4.0]

- 支持阅读统计图表，功能对齐微信读书 App 的阅读时间统计。
- 单章或全文阅读结束后，可从弹窗快速选择下一章、章节目录、书架、搜索或阅读统计。
- 修复隐藏划线和想法后，点击划线区域屏幕无响应的问题。
- 大幅提升划线和想法的加载速度。
- 支持下载过程中禁止设备休眠。
- 支持扫描本地缓存目录并添加书籍。

> **升级注意：** 本地划线和想法机制已经升级，必须重新下载带划线和想法的书籍，否则可能无法正常显示。

感谢 @shichen35、@markx 和 @jqs7 的贡献。

## [0.3.0]

- 支持二维码登录，移除繁琐的 cURL 配置流程。
- 修复阅读时间上报问题（#13）。

> **升级注意：** 升级插件后需要重新扫码登录。

感谢 @pengcw 和 @NEOS4412 的贡献。

## [0.2.1]

- 修正章节排列索引偏移一位导致的乱码问题（#29）。
- 构建章节 XHTML 时合并所有正文内容（#34）。
- 嵌入 EPUB 封面时使用高分辨率封面（#33）。
- 修复缓存 EPUB 的重复渲染循环，并显示缓存状态（#46）。
- 使用 `ffi/archiver` 生成 EPUB，防止章节较多时因内存不足导致下载失败（#44）。
- 优化书架排序入口，支持按阅读状态和下载状态筛选（#41）。
- 优化想法和划线的下载体验与渲染性能。
- 修复 Gateway API 错误携带 Web Cookie 的问题。

感谢 @jqs7、@zhouwt、@saigyouyou 和 @shichen35 的贡献。

## [0.2.0]

- 修复网络不通时加载书架卡死的问题。
- 下载书籍时支持同步下载划线。
- 支持配置书籍缓存路径。
- 支持在阅读时注入章节划线，并通过弹窗查看想法。
- 修复章节请求上下文刷新和样式参数问题。

感谢 @jqs7 和 @shichen35 的贡献。

## [0.1.1]

- 优化公众号文章排版；关闭图片下载时会移除图片及占位区域。
- 书籍与公众号图片使用独立开关，默认开启书籍图片、关闭公众号图片。
- 改进 Cookie、`x-wr-ticket` 和 `x-wrpa-0` 的续期与持久化，新配置仍具有优先级。
- 统一日志标识，优化错误信息、日志降噪及敏感内容保护。
- 重构设置菜单与缓存管理；API Key 改为仅通过 `config.lua` 配置。

## [0.1.0]

- 首次正式发布微信读书 KOReader 插件。
- 支持浏览微信读书书架、搜索书籍，以及下载单章、前若干章或整本书为 EPUB。
- 支持浏览已关注的公众号并下载文章为 HTML。
- 支持后台上报阅读时长，可自动关联当前书籍或手动指定目标书籍。
- 支持书架排序、书籍详情、EPUB 封面和缓存管理。
